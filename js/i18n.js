/* ═══════════════════════════════════════════════
   THE VAULT — i18n.js  v2
   ES / EN language system + post auto-translation
   ═══════════════════════════════════════════════ */
...
const TRANSLATIONS = {
  ...
  /* ─── POSTS (content labels used by auto-translate UI) ─── */
  ...
  'post.translating': { es: 'Traduciendo con IA...', en: 'Translating with AI...' },
  'post.translated':  { es: 'Traducido automáticamente', en: 'Auto-translated' },
  'post.original':    { es: 'Ver original', en: 'View original' },
};
...
const I18N = {
  ...
  init() {
    ...
    this.apply();
    this.renderToggle();
    // Auto-translate post content if on a post page
    if (document.getElementById('post-content')) {
      this._postOriginalHTML = document.getElementById('post-content').innerHTML;
      if (this.current === 'es') {
        this._translatePostContent('es');
      }
    }
  },
  ...
  apply() {
    ...
    this._updateTocTitle();
    this._updateBackLinks();
  },

  _updateTocTitle() {
    const tocH4 = document.querySelector('.toc h4');
    if (tocH4) tocH4.textContent = this.get('post.toc');
  },

  _updateBackLinks() {
    document.querySelectorAll('[data-back]').forEach(el => {
      const type = el.dataset.back;
      el.textContent = this.get(`post.back.${type}`) || el.textContent;
    });
  },

  toggle() {
    this.current = this.current === 'en' ? 'es' : 'en';
    localStorage.setItem('vault_lang', this.current);
    this.apply();
    // Handle post content translation
    ...
    if (postContent) {
      if (this.current === 'es') {
        ...
        // Restore original
        postContent.innerHTML = this._postOriginalHTML;
        this._removeTranslationBadge();
      }
    }
  },
  ...
  _updateToggleBtn() {
    ...
    if (this.current === 'en') {
      ...
      btn.style.background = 'rgba(0,255,245,.08)';
      btn.style.textShadow = 'var(--glow-c)';
    } else {
      btn.textContent = 'EN';
      btn.title = 'Switch to English';
      btn.style.color = 'var(--magenta)';
      btn.style.borderColor = 'var(--magenta)';
      btn.style.background = 'rgba(255,0,170,.08)';
      btn.style.textShadow = 'var(--glow-m)';
    }
  },
  ...
  async _translatePostContent(lang) {
    const postContent = document.getElementById('post-content');
    if (!postContent) return;

    // Check cache
    if (this._postTranslatedHTML[lang]) {
      postContent.innerHTML = this._postTranslatedHTML[lang];
      this._addTranslationBadge();
      return;
    }

    // Get text to translate (strip code blocks to preserve them)
    const originalHTML = this._postOriginalHTML;

    // Show loading indicator
    this._showTranslatingBadge();

    // Extract text nodes only, preserve code/pre blocks
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = originalHTML;

    // Collect text segments to translate (skip code/pre)
    const segments = [];
    ...
    // Build combined text for translation (use |||SEP||| as separator)
    const combined = textNodes.map(n => n.textContent.trim()).join('\n|||SEP|||\n');

    if (!combined.trim()) {
      this._removeTranslatingBadge();
      return;
    }
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `You are a technical cybersecurity translator. Translate the following text segments from English to Spanish. Each segment is separated by "|||SEP|||". Return ONLY the translated segments separated by "|||SEP|||" in the exact same order. Do NOT translate: technical terms (like LSASS, Mimikatz, SOC, TTPs, IOCs, CTF, SIEM, EDR), code, command names, tool names, proper nouns, URLs, file paths, or flag values. Keep the same tone (technical, concise). Do not add explanations.
...
${combined}`
      ...
      const translatedSegments = translatedText.split('|||SEP|||').map(s => s.trim());

      if (translatedSegments.length === textNodes.length) {
        textNodes.forEach((node, i) => {
          if (translatedSegments[i]) node.textContent = translatedSegments[i];
        });
        this._postTranslatedHTML[lang] = tempDiv.innerHTML;
      ...
      this._removeTranslatingBadge();
      postContent.innerHTML = originalHTML; // Restore on failure
    }
  },

  _showTranslatingBadge() {
    this._removeTranslatingBadge();
    const badge = document.createElement('div');
    badge.id = 'translation-badge';
    badge.style.cssText = `
      position: fixed; bottom: 24px; right: 24px; z-index: 9999;
      font-family: var(--font-mono); font-size: .62rem; letter-spacing: .12em;
      background: var(--dark2); border: 1px solid var(--magenta);
      color: var(--magenta); padding: 8px 14px;
      clip-path: polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px));
      animation: pulse 1.5s ease-in-out infinite;
    `;
    badge.textContent = '⟳ ' + this.get('post.translating');
    if (!document.getElementById('translation-keyframes')) {
      const style = document.createElement('style');
      style.id = 'translation-keyframes';
  ...
  _removeTranslatingBadge() {
    const old = document.getElementById('translation-badge');
    if (old) old.remove();
  },

  _addTranslationBadge() {
    this._removeTranslatingBadge();
    const badge = document.createElement('div');
    badge.id = 'translation-badge';
    badge.style.cssText = `
      ...
      background: var(--dark2); border: 1px solid rgba(0,255,245,.3);
      color: var(--text-dim); padding: 8px 14px;
      clip-path: polygon(0 0,calc(100% - 6px) 0,100% 6px,100% 100%,6px 100%,0 calc(100% - 6px));
    `;
    badge.innerHTML = `✓ ${this.get('post.translated')}`;
    document.body.appendChild(badge);
    setTimeout(() => { if (badge.parentNode) badge.style.opacity = '0'; badge.style.transition = 'opacity 1s'; setTimeout(() => badge.remove(), 1000); }, 4000);
  },
};
